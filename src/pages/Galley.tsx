...
const TaskItem = React.memo(({ icon, name }: { icon: string; name: string }) => {
  return (
    <div
      className="flex items-center rounded-[8px] mb-[21px] px-[22px] py-[11px] pr-[33px]"
      style={{
        background:
    ...
    >
      ...
      <span className="flex-1 truncate text-white" style={{ color: 'white' }}>
        {name}
      </span>
      <button className="rounded-full bg-[#47E49F] text-[#134631] px-[15px] py-[9px]">
        To finish
      </button>
    </div>
  );
...
const NFTbody = React.memo(() => {
  return (
    <>
      ...
      <div className="mt-[20px] border-[3px] rounded-[40px] border-[#3EE19E] px-[34px] py-[23px]">
        X data
      </div>
    </>
  );
    ...
    <div className="w-full h-full flex justify-center">
      <div className="max-w-[1920px] flex justify-center">
        <div className="w-[425px] h-[607px] mt-[140px] mr-[120px] relative">
          <div
            className="absolute w-full h-full p-[20px] bg-center bg-cover z-[10]"
