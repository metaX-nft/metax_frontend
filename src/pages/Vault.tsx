...
import {
  ...
  useClaimXToken,
  useUnStakeETH,
} from '@abis/contracts/stake/StakeContract';
...
const Vault = React.memo(() => {
  ...
  // stake eth
  const { sendStakeInput, sending, sendSuccess, stakeError, hash } = useSendStakeWork();

  const handleSendStakeWork = async () => {
    try {
      await sendStakeInput([], { value: parseEther(inputValue) });
    } catch (e) {
      console.log('e', e);
    }
  };
  ...
  useEffect(() => {
    if (hash && (!sendSuccess || sendSuccess)) {
      setInputValue('1');
    }
  }, [sendSuccess, hash, sending]);
    ...
    <div className="w-full h-full flex justify-center items-center">
      <div className="w-[960px] h-[616px] relative z-[10]">
        ...
        <div className="absolute top-0 left-0 w-full h-full">
          <p className="font-[600] text-[32px] leading-[48px] px-[60px] py-[16px] text-white">
            Stake
          </p>
          ...
          <div className="pt-[28px] pl-[60px] flex">
            <div className="rounded-[34px] border-[2px] border-[#47E49F] w-[500px] h-[64px] flex items-center px-[22px] justify-between">
              ...
              <input
                ...
                className="w-[430px] bg-[unset] text-white text-right leading-[29px] text-[24px] font-[400]"
              />
            </div>
            <button
              className="rounded-[38px] w-[263px] h-[64px] bg-[#47E49F] ml-[50px]"
              onClick={handleSendStakeWork}
              disabled={sending}
            >
              <span className="font-[400] text-[24px] leading-[29px]">
                {sending ? 'sending' : 'Stake ETH'}
              </span>
            </button>
          </div>
          ...
          <div className="flex justify-center mt-[37px]">
            <div className="w-[380px] h-[350px] border-[2px] border-[#47E49F] rounded-[40px] mr-[47px] py-[40px] flex flex-col items-center">
              ...
              <p className="mt-[21px] text-white text-[32px] leading-[39px] font-[700]">
                {StakedETH as number}
              </p>
              <button className="mt-[36px] rounded-[32px] w-[264px] h-[54px] bg-[#47E49F]">
                <span className="text-[24px] leading-[29px] font-[400]" onClick={handleUnStake}>
            ...
            <div className="w-[380px] h-[350px] border-[2px] border-[#47E49F] rounded-[40px] py-[40px] flex flex-col items-center">
              ...
              <button
                className="mt-[96px] rounded-[32px] w-[264px] h-[54px] bg-[#47E49F]"
                onClick={handleClaim}
              >
